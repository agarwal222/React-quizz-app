...
function App() {
  ...
  useEffect(() => {
    ...
    quizData.map((item, index) => {
      const parsedCorrectAnswers = new DOMParser().parseFromString(
        item.correct_answer,
        "text/html"
      );
  ...
  const quizQuestions = buttonArray.map((item) => {
    const parsedQuestions = new DOMParser().parseFromString(
      item.question,
      "text/html"
    );
    ...
    const answerButtons = item.answers.map((item) => {
      const parsedAnswers = new DOMParser().parseFromString(
        item.name,
        "text/html"
      );
